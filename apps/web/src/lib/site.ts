 ...
 *
 * Termi is self-hosted: every instance runs on its own domain, configured via
 ...
 * self-hoster's pages would declare `termi.shuvoo.com` as their canonical URL,
...
export function getSiteUrl(): string {
    ...
    const base = configured || 'https://termi.shuvoo.com';
    return base.replace(/\/+$/, '');
}
