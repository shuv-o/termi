...
function computeScores(
    ...
    const singlePts = cpu ? Math.min(1000, Math.round((cpu.singleCoreMBps * 1000) / 800)) : 0;
    const multiPerThread = cpu && threads > 0 ? cpu.multiCoreMBps / threads : 0;
    const multiPts = cpu ? Math.min(1000, Math.round((multiPerThread * 1000) / 800)) : 0;
    ...
    return new Promise((resolve) => {
        ...
        client.on('ready', async () => {
            try {
                ...
                onProgress({
                    phase: 'cpu_single',
                    message: 'Measuring single-core CPU speed (3 s)…',
                });
                ...
                onProgress({
                    phase: 'cpu_multi',
                    message: `Measuring multi-core CPU speed (${threads} threads, 3 s)…`,
                });
                ...
                onProgress({
                    phase: 'ram_write',
                    message: 'Benchmarking RAM write speed (256 MB)…',
                });
                ...
                if (ramWriteMBps || ramReadMBps) {
                    ...
                    onProgress({
                        phase: 'ram_read',
                        message: '/dev/shm unavailable — RAM test skipped',
                    });
                }
                ...
                onProgress({
                    phase: 'disk_write',
                    message: 'Benchmarking disk write speed (256 MB)…',
                });
                ...
                onProgress({
                    phase: 'disk_read',
                    message: 'Benchmarking disk read speed (256 MB)…',
                });
                ...
                onProgress({
                    phase: 'network',
                    message: 'Running network tests (ping + loopback)…',
                });
                ...
                const netMsg =
                    [
                        ...
                        loopbackMBps != null
                            ? `${loopbackMBps.toLocaleString()} MB/s loopback`
                            : null,
                    ]
                        .filter(Boolean)
                        .join(', ') || 'Network test complete';
