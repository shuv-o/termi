...
export default function InstallPrompt() {
    ...
    return (
        <div className="fixed inset-x-0 bottom-0 z-[60] flex justify-center px-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
            <div className="flex w-full max-w-md items-center gap-3 rounded-2xl border border-border/60 bg-background/95 p-3 shadow-xl backdrop-blur supports-[backdrop-filter]:bg-background/80">
                ...
                <div className="min-w-0 flex-1 text-sm">
                    {showIOS ? (
                        <p className="text-foreground/90">
                            Install Termi: tap{' '}
                            <Share className="inline h-3.5 w-3.5 -translate-y-0.5 text-muted-foreground" />{' '}
                ...
                {!showIOS && (
                    <button
                        type="button"
                        onClick={install}
                        className="shrink-0 rounded-lg bg-linear-to-r from-primary to-sky-600 px-3.5 py-2 text-sm font-semibold text-white transition-transform hover:-translate-y-0.5"
                    >
                        Install
                    </button>
                )}
                ...
                <button
                    ...
                    onClick={dismiss}
                    title="Dismiss"
                    className="shrink-0 rounded-lg p-1.5 text-muted-foreground/80 transition-colors hover:bg-card hover:text-foreground/90"
                >
                    <X className="h-4 w-4" />
                </button>
            </div>
