...
const getBackoffMs = (attempt: number) => Math.min(Math.pow(1.5, attempt) * 1000, 15_000);
...
export default function TelnetTerminal({
    ...
    const connect = useCallback(async () => {
        ...
        const wsUrl = `${gatewayBase}/connect?protocol=telnet&serverId=${encodeURIComponent(serverId)}`;
        ...
        ws.onopen = async () => {
            ...
            try {
                token =
                    !hasConnectedOnceRef.current || !renewTokenRef.current
        ...
        ws.onclose = () => {
            ...
            if (retryCountRef.current < MAX_AUTO_RETRIES) {
                ...
                retryTimerRef.current = setTimeout(() => {
                    connect().catch((err) =>
                        console.error('[TelnetTerminal] reconnect error:', err),
                    );
                }, delayMs);
            } else {
                updateStatus('disconnected');
    ...
    }, [serverId, gatewayUrl, updateStatus]);
    ...
    const reconnect = useCallback(() => {
        ...
        if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
            ...
            connect().catch((err) =>
                console.error('[TelnetTerminal] manual reconnect error:', err),
            );
        }
    }, [connect]);
    ...
    useEffect(() => {
        ...
        terminal.write('Connecting to Telnet server...\r\n');
        retryCountRef.current = 0;
        connect().catch((err) => console.error('[TelnetTerminal] initial connect error:', err));
