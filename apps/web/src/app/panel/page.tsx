...
import {
    ...
    ArrowUpDown, Zap, Tag, X, Share2, UserPlus, Loader2, Mail, Shield, Unlink,
} from 'lucide-react';
import {
    Dialog,
    DialogContent,
...
function ShareModal({ server, onClose }: { server: ServerItem; onClose: () => void }) {
    ...
    const handleInvite = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!d.success) { setError(d.error || 'Failed to send invitation'); return; }
    ...
    const handleRevoke = async (id: string, isInvitation = false) => {
        ...
        setData(prev => prev ? {
            ...
            pending: isInvitation ? prev.pending.filter(p => p.id !== id) : prev.pending,
        } : null);
    };
                ...
                {loadingShares ? (
                    ...
                    <div className="space-y-1.5">
                        ...
                        {data?.shares.map(share => (
                            <div key={share.id} className="flex items-center gap-2.5 px-3 py-2 rounded-lg bg-secondary/60 border border-border/50">
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="text-xs font-medium truncate">{share.sharedWith.email}</p>
                                    <p className="text-[10px] text-emerald-400">Active · {share.permissions}</p>
                                </div>
                                <button
                        ...
                        {data?.pending.map(inv => (
                            <div key={inv.id} className="flex items-center gap-2.5 px-3 py-2 rounded-lg bg-secondary/40 border border-border/40 opacity-70">
                                <div className="w-7 h-7 rounded-full bg-secondary flex items-center justify-center text-muted-foreground shrink-0">
                                    <Mail className="w-3.5 h-3.5" />
                                </div>
                                <div className="flex-1 min-w-0">
                                    <p className="text-xs font-medium truncate">{inv.inviteeEmail}</p>
                                    <p className="text-[10px] text-yellow-400">Pending invitation · {inv.permissions}</p>
                                </div>
                                <button
...
function GridCard({
    ...
    onFavorite, onEdit, onDelete, onCopyPassword, onConnect, onSessions, onTagClick, onShare,
}: {
    server: ServerItem;
...
export default function DashboardPage() {
    ...
    useEffect(() => {
        fetch('/api/shared-servers')
            .then(r => r.json())
            .then(d => { if (d.success) setSharedServers(d.data.servers); })
            .catch(() => {});
    }, []);
        ...
        {sharedServers.length > 0 && (
            <div className="max-w-6xl mx-auto mt-8">
                <div className="flex items-center gap-2 mb-4">
                    <Share2 className="w-4 h-4 text-primary" />
                    <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Shared with me</h2>
                    <span className="text-xs bg-primary/15 text-primary px-1.5 py-0.5 rounded-full font-medium">{sharedServers.length}</span>
                </div>
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {sharedServers.map(server => {
                        const Icon = protocolIcons[server.protocol];
                        return (
                            <Card key={server.id} className="flex flex-col overflow-hidden bg-card border-border border-primary/20 hover:border-primary/40 hover:shadow-lg hover:shadow-primary/5 transition-all duration-200">
                                ...
                                <div className="px-4 pb-4">
                                    <Button
                                        ...
                                        onClick={() => router.push(`/panel/connect/${server.id}/${server.protocol.toLowerCase()}`)}
                                    >
                                        Connect
                                    </Button>
                                </div>
