                            ...
                            <div className="flex flex-wrap gap-1.5">
                                {PRESET_ICONS.map(({ value, label, icon: Icon }) => (
                                    <button
                                        ...
                                        title={label}
                                        onClick={() =>
                                            update({ icon: form.icon === value ? '' : value })
                                        }
...
export default function GroupsPage() {
    ...
    const handleEdit = async (form: GroupFormData) => {
        ...
        setGroups((prev) =>
            prev.map((g) => (g.id === editTarget.id ? { ...g, ...data.data.group } : g)),
        );
        setDetails((prev) => {
            const next = { ...prev };
    ...
    const handleConnect = useCallback(
        (serverId: string, protocol: string) => {
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-4rem)] lg:h-[calc(100vh-0rem)] -m-4 lg:-m-8">
                ...
                <div className="flex items-center gap-4">
                    <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                            {loading
                                ? 'Loading…'
                                : `${groups.length} group${groups.length !== 1 ? 's' : ''} · ${totalServers} server${totalServers !== 1 ? 's' : ''}`}
                        </p>
                    </div>
                </div>
            ...
            <div className="flex flex-1 min-h-0 overflow-hidden">
                                        ...
                                        >
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex items-center gap-1.5 text-[11px] text-muted-foreground/60">
                                                    ...
                                                    <span>
                                                        {group._count.servers} server
                                                        {group._count.servers !== 1 ? 's' : ''}
                                                    </span>
                                                </div>
                                            </div>
                                            ...
                                            >
                                                ...
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent
                                                        align="end"
                                                        className="w-36 bg-card border-border text-xs"
                                                    >
                                                        <DropdownMenuItem
                ...
                <div
                    className={`flex-1 min-w-0 min-h-0 overflow-hidden ${selectedGroup ? '' : 'hidden lg:flex'}`}
                >
                    <GroupDetailPanel
                        group={selectedGroup}
