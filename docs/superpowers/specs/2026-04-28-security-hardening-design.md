...
**Fix:** Change to `|| process.env.GATEWAY_URL || 'ws://localhost:8080'`. Also align `GATEWAY_URL` vs `NEXT_PUBLIC_GATEWAY_URL` usage in `.env.example` with clear comments on which is server-side only.
